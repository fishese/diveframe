...
- Refresh with `npm run bundle:overlay-fonts` (Node only); rebundle now
...
- Android debug APK **1.0.14** (`versionCode 15`) built from `37e7dd0`.
  Airplane-mode Compose smoke testing confirmed that preview overlays render
...
- Device airplane-mode Compose: Export image / Ready preview showed overlay
  duration, depth, temperature, and gas labels.
