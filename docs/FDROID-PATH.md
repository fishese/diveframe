...
Status: build preparation. The current public Android build is the signed GitHub
nightly channel (`diveframe-nightly.apk`, documented in
`docs/WEB-APK-SYNC.md`). F-Droid metadata belongs in the separate `fdroiddata`
repository and should point to a full release commit from this repository.
...
- `/android`, the README, and the current What's new entry use the signed
  GitHub nightly URL until an F-Droid listing is available:
  `releases/download/nightly/diveframe-nightly.apk`.
- Nightly version names and codes are generated by the GitHub Actions run.
  F-Droid should receive deliberate release versions after accumulated changes,
  rather than every nightly build.
