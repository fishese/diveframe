...
test("Preview Android identity is isolated from production", async () => {
  ...
  assert.match(buildGradle, /defaultConfig\s*\{[\s\S]*versionCode 30[\s\S]*versionName "1\.0\.29"/);
  assert.match(buildGradle, /^\s*versionCode 30$/m);
  assert.match(buildGradle, /^\s*versionName "1\.0\.29"$/m);
...
test("Android launcher exposes a localized Memos shortcut to /memo", async () => {
  ...
  assert.match(mainActivity, /"\/\?dive=" \+ Uri\.encode\(dive\)/);
});
